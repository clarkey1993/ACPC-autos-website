...
@php
    ...
    $phoneCtaLabel = $label ?? __('public.buttons.call_us');
...
@if ($phoneCtaVariant === 'inline-link')
    ...
    <span class="phone-static-inline fw-semibold d-none d-md-inline" aria-label="{{ __('public.contact.phone_aria') }}">{{ $publicPhoneDisplay }}</span>
@else
    ...
    <span class="btn btn-brand-primary phone-cta-static d-none d-md-inline-flex align-items-center justify-content-center"
          role="text"
          aria-label="{{ __('public.contact.phone_aria') }} {{ $publicPhoneDisplay }}">
        {{ $publicPhoneDisplay }}
    </span>
@endif
